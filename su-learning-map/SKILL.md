...
Build a reusable learning system, not merely a summary. Keep evidence, retrieval, callable behavior, learning content, visual presentation, and deployment as separate layers with explicit handoffs.

The default engineering chain is:

```text
PDF / source material
→ page-anchored Markdown evidence
→ RAG retrieval
→ subject Skill
→ learning map
→ optional Cloudflare Pages delivery
```

The user-facing outcome is a personal knowledge asset that can be reviewed, verified, called, accumulated, and productized.
...
For the copy-ready installation and orchestration prompts, plus field defaults, read `references/quick-invocation.md`. Infer omitted fields from the supplied material and current context. Pause only at the checkpoints listed below; do not ask the user to restate defaults already encoded in this Skill.
...
- Page-anchored Markdown is the source of truth for retrieval; the original source remains the quotation authority.
